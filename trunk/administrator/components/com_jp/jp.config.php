...
<?php
        ...
        if ($form)
            echo '<form action="index2.php" method="post" id="adminForm" name="adminForm" enctype="">';
        //echo "<br>";
        $option = mosGetParam($_REQUEST, 'option', false);
        if (!$option)
        ...
        */
        ...
        foreach ($rows as $k => $row) {
            ...
            if ($set != $row->fieldset) {
                ...
                //   $tabs .= '</fieldset>';
                //}
                echo '<fieldset style="display:block; width:5%; float:left; clear:both"><LEGEND>' .
                    ucfirst($row->fieldset) . '</LEGEND><table width="20%" class="admintable">';
            }
            $com = $row->section;
            $set = $row->fieldset;
            //$tabs .= "<P>Get it</P>";
    ...
    function _renderRow($row)
    {
        $label = $row->lable;
        if ($row->description) {
            $label = sprintf('<SPAN class="editlinktip"><SPAN onmouseover="return ' .
        ...
        $out = sprintf('<tr valign="top"><td width="10%s" nowrap="nowrap">%s:</td><td width="10%s" nowrap="nowrap">%s</td></tr>',
            '%', $label, '%', JEConfig::_renderElement($row));
        return $out;
    }
    ...
    function _renderElement($row)
    {
        switch ($row->type) {
            case "yesno":
                $out = mosHTML::yesnoRadioList($row->name . $row->id, 'class="inputbox"', $row->
                    selected);
                break;
            ...
            case "selectlist":
                ...
                $out = mosHTML::selectList($list, $row->name . $row->id,
                    'class="inputbox" size="' . count($items) . '" ', 'value', 'text', $row->
                    selected);
                break;
            ...
            case "dropdown":
                ...
                $out = mosHTML::selectList($list, $row->name . $row->id,
                    'class="inputbox" size="1" ', 'value', 'text', $row->selected);
                break;

            case "textarea":
                $out = sprintf('<textarea class="text_area" name="%s" rows="5" cols="40">%s</textarea>',
                    $row->name . $row->id, $row->selected);
                break;

            default:
                $out = sprintf('<input type="text" class="inputbox" name="%s" value="%s" size="40">',
                    $row->name . $row->id, $row->selected);
                break;
        }
