...
class GoogleSearch
{
    ...
    function doGetCachedPage($url = '')
    {
        ...
        $this->queryParams = array('key' => $this->key, 'url' => $this->url);
    ...
    function doSearch()
    {
        ...
        $this->queryParams = array('key' => $this->key, 'q' => $this->q, 'start' => $this->
            start, 'maxResults' => $this->maxResults, 'filter' => $this->filter, 'restrict' =>
            $this->restrict, 'safeSearch' => $this->safeSearch, 'lr' => $this->lr, 'ie' => $this->
            ie, //deprecated
    ...
    function doSpellingSuggestion($phrase = '')
    {
        ...
        $this->queryParams = array('key' => $this->key, 'phrase' => $this->phrase);
    ...
    {
        $soapclient = new soapclient2($this->wsdlURL, 'wsdl', $this->nameSpace);
...
class nusoap_base
{
    ...
    var $namespaces = array('SOAP-ENV' =>
        ...
        'http://schemas.xmlsoap.org/soap/encoding/', 'si' =>
     ...
     * @access   public
     */
    var $typemap = array('http://www.w3.org/2001/XMLSchema' => array('string' =>
        'string', 'boolean' => 'boolean', 'float' => 'double', 'double' => 'double',
        'decimal' => 'double', 'duration' => '', 'dateTime' => 'string', 'time' =>
        'string', 'date' => 'string', 'gYearMonth' => '', 'gYear' => '', 'gMonthDay' =>
        '', 'gDay' => '', 'gMonth' => '', 'hexBinary' => 'string', 'base64Binary' =>
        'string', // derived datatypes
        'normalizedString' => 'string', 'token' => 'string', 'language' => '', 'NMTOKEN' =>
        '', 'NMTOKENS' => '', 'Name' => '', 'NCName' => '', 'ID' => '', 'IDREF' => '',
        'IDREFS' => '', 'ENTITY' => '', 'ENTITIES' => '', 'integer' => 'integer',
        'nonPositiveInteger' => 'integer', 'negativeInteger' => 'integer', 'long' =>
        'integer', 'int' => 'integer', 'short' => 'integer', 'byte' => 'integer',
        'nonNegativeInteger' => 'integer', 'unsignedLong' => '', 'unsignedInt' => '',
        'unsignedShort' => '', 'unsignedByte' => '', 'positiveInteger' => ''),
        'http://www.w3.org/1999/XMLSchema' => array('i4' => '', 'int' => 'integer',
        'boolean' => 'boolean', 'string' => 'string', 'double' => 'double', 'float' =>
        'double', 'dateTime' => 'string', 'timeInstant' => 'string', 'base64Binary' =>
        'string', 'base64' => 'string', 'ur-type' => 'array'),
        'http://soapinterop.org/xsd' => array('SOAPStruct' => 'struct'),
        'http://schemas.xmlsoap.org/soap/encoding/' => array('base64' => 'string',
        'array' => 'array', 'Array' => 'array'), 'http://xml.apache.org/xml-soap' =>
        array('Map'));
     ...
     * @access   public
     */
    var $xmlEntities = array('quot' => '"', 'amp' => '&', 'lt' => '<', 'gt' => '>',
        'apos' => "'");
     ...
     * @access	public
     */
    function serialize_val($val, $name = false, $type = false, $name_ns = false, $type_ns = false,
        $attributes = false)
    {
        ...
        switch (true) {
            ...
            case is_object($val):
                ...
                foreach (get_object_vars($val) as $k => $v) {
                    $pXml = isset($pXml) ? $pXml . $this->serialize_val($v, $k) : $this->
                        serialize_val($v, $k);
                }
                $xml .= '<' . $name . '>' . $pXml . '</' . $name . '>';
                break;
            ...
            case (is_array($val) || $type):
                ...
                if ($valueType == 'arraySimple' || ereg('^ArrayOf', $type)) {
                    ...
                    $xml = "<$name xsi:type=\"SOAP-ENC:Array\" SOAP-ENC:arrayType=\"" . $array_typename .
                        "[$array_type]\"$atts>" . $xml . "</$name>";
                } else {
                    // got a struct
    ...
    function serializeEnvelope($body, $headers = false, $namespaces = array())
    {
        ...
        return '<?xml version="1.0" encoding="' . $this->soap_defencoding . '"?' . ">" .
            ...
            $ns_string . ">" . $headers . "<SOAP-ENV:Body>" . $body . "</SOAP-ENV:Body>" .
            "</SOAP-ENV:Envelope>";
    }
...
function timestamp_to_iso8601($timestamp, $utc = true)
{
    ...
    if ($utc) {
        ...
        if (ereg($eregStr, $datestr, $regs)) {
            return sprintf('%04d-%02d-%02dT%02d:%02d:%02dZ', $regs[1], $regs[2], $regs[3], $regs[4],
                $regs[5], $regs[6]);
        }
        return false;
    } else {
...
class soap_fault extends nusoap_base
{
    ...
    function serialize()
    {
        ...
        $return_msg = '<?xml version="1.0"?' . ">\n" .
            '<SOAP-ENV:Envelope SOAP-ENV:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"' .
            $ns_string . ">\n" . '<SOAP-ENV:Body>' . '<SOAP-ENV:Fault>' . '<faultcode>' . $this->
            faultcode . '</faultcode>' . '<faultactor>' . $this->faultactor .
            '</faultactor>' . '<faultstring>' . $this->faultstring . '</faultstring>' .
...
class XMLSchema extends nusoap_base
{
    ...
    function parseString($xml, $type)
    {
        ...
        if ($xml != "") {
            ...
            if (!xml_parse($this->parser, $xml, true)) {
                ...
                $errstr = sprintf('XML error on line %d: %s', xml_get_current_line_number($this->
                    parser), xml_error_string(xml_get_error_code($this->parser)));
    ...
    function serializeSchema()
    {
        ...
        foreach ($this->complexTypes as $typeName => $attrs) {
            ...
            // compositors
            if (isset($attrs['compositor']) && $attrs['compositor'] != '' && $attrs['compositor'] !=
                'all') {
                $contentStr = "<$schemaPrefix:$attrs[compositor]>" . $contentStr . "</$schemaPrefix:$attrs[compositor]>";
            }
            // finalize complex type
            if ($contentStr != '') {
                $contentStr = "<$schemaPrefix:complexType name=\"$typeName\">" . $contentStr .
        ...
        // finish 'er up
        $xml = "<$schemaPrefix:schema xmlns=\"$this->XMLSchemaVersion\" targetNamespace=\"$this->schemaTargetNamespace\">" .
        ...
        if ($typeDef = $this->getTypeDef($type)) {
            ...
            if ($typeDef['phpType'] == 'struct') {
                ...
                foreach ($typeDef['elements'] as $child => $childDef) {
                    $buffer .= "
					<tr><td align='right'>$childDef[name] (type: " . $this->getLocalPart($childDef['type']) .
                        "):</td>
     ...
     *
     */
    function addComplexType($name, $typeClass = 'complexType', $phpType = 'array', $compositor =
        '', $restrictionBase = '', $elements = array(), $attrs = array(), $arrayType =
        '')
    {
        $this->complexTypes[$name] = array('name' => $name, 'typeClass' => $typeClass,
            'phpType' => $phpType, 'compositor' => $compositor, 'restrictionBase' => $restrictionBase,
            'elements' => $elements, 'attrs' => $attrs, 'arrayType' => $arrayType);
    }
}
...
class soapval extends nusoap_base
{
    ...
    function serialize()
    {
        return $this->serialize_val($this->value, $this->name, $this->type, $this->
            element_ns, $this->type_ns, $this->attributes);
    }
...
class soap_transport_http extends nusoap_base
{
    ...
    function connect($timeout)
    {
        ...
        // test pointer
        if (!$fp) {
            $this->debug('Couldn\'t open socket connection to server ' . $this->url .
                ', Error: ' . $this->error_str);
            $this->setError('Couldn\'t open socket connection to server: ' . $this->url .
                ', Error: ' . $this->error_str);
            return false;
        }
        return $fp;
    ...
    function send($data, $timeout = 0)
    {
        ...
        // make payload
        $this->outgoing_payload .= "User-Agent: $this->title/$this->version\r\n" .
            "Host: " . $this->host . "\r\n";
        // http auth
        $credentials = '';
        if ($this->username != '') {
        ...
        // set content type
        $this->outgoing_payload .= 'Content-Type: text/xml; charset="' . $this->
        ...
        $this->debug('received ' . strlen($this->incoming_payload) .
        ...
        if (isset($headers['Transfer-Encoding']) && $headers['Transfer-Encoding'] ==
            'chunked') {
        ...
        if (isset($headers['Content-Encoding']) && $headers['Content-Encoding'] != '') {
            if ($headers['Content-Encoding'] == 'deflate' || $headers['Content-Encoding'] ==
                'gzip') {
                ...
                if (function_exists('gzinflate')) {
                    ...
                    if ($headers['Content-Encoding'] == 'deflate' && $degzdata = @gzinflate($data)) {
                        $data = $degzdata;
                    } elseif ($headers['Content-Encoding'] == 'gzip' && $degzdata = gzinflate(substr
                    ($data, 10))) {
                        $data = $degzdata;
                    } else {
...
class soap_server extends nusoap_base
{
    ...
    function parse_request($data = '')
    {
        ...
        if (function_exists('getallheaders')) {
            ...
            if (strpos($this->headers['Content-Type'], '=')) {
                $enc = str_replace('"', '', substr(strstr($this->headers["Content-Type"], '='),
                    1));
        ...
        if ($err = $parser->getError()) {
            ...
            if ($this->verify_method($this->methodname, $request_data)) {
                ...
                $this->debug("done calling method: $this->methodname, received $method_response of type" .
                    gettype($method_response));
                ...
                $payload = '<' . $this->methodname . "Response>" . $return_val . '</' . $this->
                    methodname . "Response>";
                ...
                if ($this->wsdl) {
                    ...
                    return $this->serializeEnvelope($payload, $this->responseHeaders, $this->wsdl->
                        usedNamespaces);
                } else {
    ...
    {
        $this->operations[$methodname] = array('name' => $methodname, 'in' => $in, 'out' =>
            $out);
    }
     ...
     * @access   public
     */
    function register($name, $in = false, $out = false, $namespace = false, $soapaction = false,
        $style = false, $use = false)
    {
        if (false == $in) {
        }
        if (false == $out) {
        ...
        $this->operations[$name] = array('name' => $name, 'in' => $in, 'out' => $out,
            'namespace' => $namespace, 'soapaction' => $soapaction, 'style' => $style);
        if ($this->wsdl) {
    ...
    function webDescription()
    {
		...
		<div class=content>
			...
			<div class=nav>
				<p>View the <a href="' . $GLOBALS['PHP_SELF'] .
     ...
     */
    function configureWSDL($serviceName, $namespace = false, $endpoint = false, $style =
        'rpc', $transport = 'http://schemas.xmlsoap.org/soap/http')
    {
        ...
        $this->wsdl->bindings[$serviceName . 'Binding'] = array('name' => $serviceName .
            'Binding', 'style' => $style, 'transport' => $transport, 'portType' => $serviceName .
            'PortType');
        $this->wsdl->ports[$serviceName . 'Port'] = array('binding' => $serviceName .
            'Binding', 'location' => $endpoint, 'bindingType' =>
...
class wsdl extends XMLSchema
{
    ...
    function parseWSDL($wsdl = '')
    {
        ...
        if (isset($wsdl_props['host'])) {
            ...
            if ($fp = fsockopen($wsdl_props['host'], $wsdl_url_port, $fsockopen_errnum, $fsockopen_errstr,
                $fsockopen_timeout)) {
        ...
        foreach ($this->bindings as $binding => $bindingData) {
            if (isset($bindingData['operations']) && is_array($bindingData['operations'])) {
                foreach ($bindingData['operations'] as $operation => $data) {
                    ...
                    $this->bindings[$binding]['operations'][$operation]['input'] = isset($this->
                        bindings[$binding]['operations'][$operation]['input']) ? array_merge($this->
                        bindings[$binding]['operations'][$operation]['input'], $this->portTypes[$bindingData['portType']][$operation]['input']) :
                        $this->portTypes[$bindingData['portType']][$operation]['input'];
                    $this->bindings[$binding]['operations'][$operation]['output'] = isset($this->
                        bindings[$binding]['operations'][$operation]['output']) ? array_merge($this->
                        bindings[$binding]['operations'][$operation]['output'], $this->portTypes[$bindingData['portType']][$operation]['output']) :
                        $this->portTypes[$bindingData['portType']][$operation]['output'];
                    if (isset($this->messages[$this->bindings[$binding]['operations'][$operation]['input']['message']])) {
                        $this->bindings[$binding]['operations'][$operation]['input']['parts'] = $this->
                            messages[$this->bindings[$binding]['operations'][$operation]['input']['message']];
                    }
                    if (isset($this->messages[$this->bindings[$binding]['operations'][$operation]['output']['message']])) {
                        $this->bindings[$binding]['operations'][$operation]['output']['parts'] = $this->
                            messages[$this->bindings[$binding]['operations'][$operation]['output']['message']];
                    }
                    if (isset($bindingData['style'])) {
                        $this->bindings[$binding]['operations'][$operation]['style'] = $bindingData['style'];
                    }
                    $this->bindings[$binding]['operations'][$operation]['transport'] = isset($bindingData['transport']) ?
                        $bindingData['transport'] : '';
                    $this->bindings[$binding]['operations'][$operation]['documentation'] = isset($this->
                        portTypes[$bindingData['portType']][$operation]['documentation']) ? $this->
                        portTypes[$bindingData['portType']][$operation]['documentation'] : '';
                    $this->bindings[$binding]['operations'][$operation]['endpoint'] = isset($bindingData['endpoint']) ?
        ...
        if ($this->status == 'schema' || ereg('schema$', $name)) {
            ...
            switch ($this->status) {
                case 'message':
                    if ($name == 'part') {
                        if (isset($attrs['type'])) {
                            $this->debug("msg " . $this->currentMessage . ": found part $attrs[name]: " .
                                implode(',', $attrs));
                ...
                case 'portType':
                    switch ($name) {
                        ...
                        default:
                            ...
                            '';
                            $this->portTypes[$this->currentPortType][$this->currentPortOperation][$name]['message'] =
                                $m;
                            break;
                    }
                    break;
                case 'binding':
                    switch ($name) {
                        case 'binding':
                            ...
                            $this->bindings[$this->currentBinding] = array_merge($this->bindings[$this->
                                currentBinding], $attrs);
                            break;
                        case 'header':
                            $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->
                                opStatus]['headers'][] = $attrs;
                            break;
                        case 'operation':
                            ...
                            if (isset($attrs['style'])) {
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation]['style'] =
                                    $attrs['style'];
                            }
                            if (isset($attrs['name'])) {
                                $this->currentOperation = $attrs['name'];
                                $this->debug("current binding operation: $this->currentOperation");
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation]['name'] =
                                    $attrs['name'];
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation]['binding'] =
                                    $this->currentBinding;
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation]['endpoint'] =
                                    isset($this->bindings[$this->currentBinding]['endpoint']) ? $this->bindings[$this->
                                    currentBinding]['endpoint'] : '';
                            }
                            break;
                        case 'input':
                            $this->opStatus = 'input';
                        ...
                        case 'output':
                            ...
                            break;
                        case 'body':
                            if (isset($this->bindings[$this->currentBinding]['operations'][$this->
                                currentOperation][$this->opStatus])) {
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->
                                    opStatus] = array_merge($this->bindings[$this->currentBinding]['operations'][$this->
                                    currentOperation][$this->opStatus], $attrs);
                            } else {
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->
                                    opStatus] = $attrs;
                            }
    ...
    function end_element($parser, $name)
    {
        ...
        if ($this->documentation) {
            $this->portTypes[$this->currentPortType][$this->currentPortOperation]['documentation'] =
                $this->documentation;
    ...
    {
        $pos = isset($this->depth_array[$this->depth]) ? $this->depth_array[$this->
            depth] : 0;
        ...
        if (count($this->messages) >= 1) {
            foreach ($this->messages as $msgName => $msgParts) {
                ...
                foreach ($msgParts as $partName => $partType) {
                    ...
                    $xml .= '<part name="' . $partName . '" type="' . $typePrefix . ':' . $this->
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    $binding_xml .= '<soap:operation soapAction="' . $opParts['soapAction'] .
                        '" style="' . $attrs['style'] . '"/>';
                    $binding_xml .= '<input><soap:body use="' . $opParts['input']['use'] .
                        '" namespace="' . $opParts['input']['namespace'] . '" encodingStyle="' . $opParts['input']['encodingStyle'] .
                        '"/></input>';
                    $binding_xml .= '<output><soap:body use="' . $opParts['output']['use'] .
                        '" namespace="' . $opParts['output']['namespace'] . '" encodingStyle="' . $opParts['output']['encodingStyle'] .
                        '"/></output>';
    ...
    {
        $this->debug('in serializeRPCParameters with operation ' . $operation .
            ', direction ' . $direction . ' and ' . count($parameters) .
        ...
        // set input params
        $xml = '';
        if (isset($opData[$direction]['parts']) && sizeof($opData[$direction]['parts']) >
            0) {
    ...
    function serializeType($name, $type, $value)
    {
        ...
        if (strpos($type, ':')) {
            ...
            if ($ns == $this->XMLSchemaVersion || ($this->getNamespaceFromPrefix($ns)) == $this->
                XMLSchemaVersion) {
                ...
                // it's a scalar
                return "<$name xsi:type=\"" . $this->getPrefixFromNamespace($this->
                    XMLSchemaVersion) . ":$uqType\">$value</$name>";
            }
        } else {
            $uqType = $type;
        }
        ...
        $this->debug("serializeType: uqType: $uqType, ns: $ns, phptype: $phpType, arrayType: " .
            (isset($typeDef['arrayType']) ? $typeDef['arrayType'] : ''));
            ...
            if (is_array($this->complexTypes[$uqType]['elements'])) {
                foreach ($this->complexTypes[$uqType]['elements'] as $eName => $attrs) {
                    ...
                    if (!isset($attrs['type'])) {
                        ...
                        $xml .= $this->serialize_val($v, $eName, $this->getLocalPart($attrs['type']), null,
                            $this->getNamespaceFromPrefix($this->getPrefix($attrs['type'])));
                    }
                }
            }
            $xml .= "</$name>";
            ...
            if (is_array($value) && sizeof($value) >= 1) {
                ...
                foreach ($value as $k => $v) {
                    ...
                    if (strpos($typeDef['arrayType'], ':')) {
                        ...
                        $contents .= $this->serialize_val($v, 'item', $typeDef['arrayType'], null, $this->
                            XMLSchemaVersion);
                    }
                }
                $this->debug('contents: ' . $this->varDump($contents));
            }
            $xml = "<$name xsi:type=\"" . $this->getPrefixFromNamespace('http://schemas.xmlsoap.org/soap/encoding/') .
                ':Array" ' . $this->getPrefixFromNamespace('http://schemas.xmlsoap.org/soap/encoding/') .
                ':arrayType="' . $this->getPrefixFromNamespace($this->getPrefix($typeDef['arrayType'])) .
                ":" . $this->getLocalPart($typeDef['arrayType']) . "[$rows$cols]\">" . $contents .
                "</$name>";
        }
     ...
     * @access public 
     */
    function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction = false,
        $style = 'rpc', $use = 'encoded', $documentation = '')
    {
        ...
        // get binding
        $this->bindings[$this->serviceName . 'Binding']['operations'][$name] = array('name' =>
            $name, 'binding' => $this->serviceName . 'Binding', 'endpoint' => $this->
            endpoint, 'soapAction' => $soapaction, 'style' => $style, 'input' => array('use' =>
            $use, 'namespace' => $namespace, 'encodingStyle' => $encodingStyle, 'message' =>
            $name . 'Request', 'parts' => $in), 'output' => array('use' => $use, 'namespace' =>
            $namespace, 'encodingStyle' => $encodingStyle, 'message' => $name . 'Response',
            'parts' => $out), 'namespace' => $namespace, 'transport' =>
            'http://schemas.xmlsoap.org/soap/http', 'documentation' => $documentation);
        // add portTypes
        // add messages
        if ($in) {
            foreach ($in as $pName => $pType) {
                if (strpos($pType, ':')) {
                    $pType = $this->getNamespaceFromPrefix($this->getPrefix($pType)) . ":" . $this->
                        getLocalPart($pType);
                }
...
class soap_parser extends nusoap_base
{
    ...
    function soap_parser($xml, $encoding = 'UTF-8', $method = '')
    {
        ...
        if (!empty($xml)) {
            ...
            if (!xml_parse($this->parser, $xml, true)) {
                // Display an error message.
                $err = sprintf('XML error on line %d: %s', xml_get_current_line_number($this->
                    parser), xml_error_string(xml_get_error_code($this->parser)));
                $this->debug('parse error: ' . $err);
                $this->errstr = $err;
            } else {
                $this->debug('parsed successfully, found root struct: ' . $this->root_struct .
    ...
    function end_element($parser, $name)
    {
        ...
        if (isset($this->body_position) && $pos > $this->body_position) {
            ...
            if (isset($this->message[$pos]['attrs']['href'])) {
                ...
                $this->debug('adding data for scalar value ' . $this->message[$pos]['name'] .
                    ' of value ' . $this->message[$pos]['cdata']);
    ...
    function buildVal($pos)
    {
        ...
        $this->debug('inside buildVal() for ' . $this->message[$pos]['name'] . "(pos $pos) of type " .
            $this->message[$pos]['type']);
        ...
        if ($this->message[$pos]['children'] != '') {
            ...
            if (isset($this->message[$pos]['arrayCols']) && $this->message[$pos]['arrayCols'] !=
                ...
                // array
            } elseif ($this->message[$pos]['type'] == 'array' || $this->message[$pos]['type'] ==
            'Array') {
...
class soapclient2 extends nusoap_base
{
    ...
    function soapclient2($endpoint, $wsdl = false)
    {
        ...
        if ($wsdl) {
            ...
            if ($errstr = $this->wsdl->getError()) {
                ...
                $this->debug('got ' . count($this->operations) . ' operations from wsdl ' . $this->
                    wsdlFile);
            } else {
     ...
     * @access   public
     */
    function call($operation, $params = array(), $namespace = '', $soapAction = '',
        $headers = false)
    {
        ...
        if ($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)) {
            ...
            $namespace = isset($opData['input']['namespace']) ? $opData['input']['namespace'] :
                'http://testuri.org';
            $style = $opData['style'];
            // add ns to ns array
            ...
            if ($opData['input']['use'] == 'literal') {
                ...
                $payload = "<" . $this->wsdl->getPrefixFromNamespace($namespace) . ":$operation>" .
                    $this->wsdl->serializeRPCParameters($operation, 'input', $params) . '</' . $this->
                    wsdl->getPrefixFromNamespace($namespace) . ":$operation>";
            }
            $this->debug('payload size: ' . strlen($payload));
            // serialize envelope
            $soapmsg = $this->serializeEnvelope($payload, $this->requestHeaders, $this->
                wsdl->usedNamespaces);
            $this->debug("wsdl debug: \n" . $this->wsdl->debug_str);
